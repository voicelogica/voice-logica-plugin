...
3. Test with a Call ID or a one-off `send_sms`, then `get_sms`.
...
The sender and route are working. The payload encoding is wrong. Resend with proper UTF-8 text. Do not rotate the sender or rewrite the prompt as the first step.
